...
/**
 * Loads the sample test input for the current puzzle.
 */
...
/**
 * Loads the sample puzzle input for the current puzzle.
 */
async function loadPuzzleInput() {
  await stopSolvingPuzzle();
  taskInput.value = await (await fetch(`./${years[currentYearIndex].days[currentDayIndex].path}/puzzleInput.txt`)).text();
  solutionConsole.clear();
  visualizationContainer.innerHTML = "";
}
